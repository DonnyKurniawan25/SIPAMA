			...
			<div class="card-body">
				...
				if (!empty($nilaicfP2)) {
					...
					?>
						...
						<?php
						// Buat sebuah array untuk menyimpan bukti atau informasi
						$evidence = array(
							"A" => 0.7, // Bukti A dengan key A dan nilai 0.7
							"B" => 0.4, // Bukti B dengan key B dan nilai 0.4
							"C" => 0.5 // Bukti C dengan key C dan nilai 0.5
						);

						// Hitung total dari semua nilai bukti
						$total = array_sum($evidence);

						// Hitung nilai probabilitas untuk setiap kombinasi dari bukti
						$probabilities = array();
						foreach ($evidence as $key1 => $value1) {
							foreach ($evidence as $key2 => $value2) {
								if ($key1 != $key2) {
									$probabilities[$key1 . $key2] = ($value1 + $value2 - ($value1 * $value2)) / (1 - $total);
								}
							}
						}

						// Buat sebuah array untuk menyimpan nilai kesimpulan dari hasil Dempster-Shafer
						$beliefs = array();
						foreach ($evidence as $key => $value) {
							$beliefs[$key] = $value;
						}

						// Kombinasikan nilai probabilitas untuk mendapatkan nilai kesimpulan
						foreach ($probabilities as $key => $value) {
							$keys = str_split($key);
							$newBelief = (($beliefs[$keys[0]] * $beliefs[$keys[1]]) + ($beliefs[$keys[0]] * (1 - $beliefs[$keys[1]]) * $value) + ($beliefs[$keys[1]] * (1 - $beliefs[$keys[0]]) * $value)) / (1 - ((1 - $beliefs[$keys[0]]) * (1 - $beliefs[$keys[1]]) * $value));
							$beliefs[$key] = $newBelief;
						}

						// Print hasil kesimpulan
						foreach ($beliefs as $key => $value) {
							echo "Nilai kesimpulan untuk $key: $value \n";
						}
						?>
